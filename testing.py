...
from auxiliary.dataset import Pix3D, Pascal3D, Linemod, Pascal3DContrast, Pix3DContrast
...
parser.add_argument('--shape_feature_dim', type=int, default=1024, help='feature dimension for shapes')
parser.add_argument('--bin_size', type=int, default=15, help='bin size for the euler angle classification')

# dataset settings
parser.add_argument('--dataset', type=str, default=None,
                    choices=['ObjectNet3D', 'Pascal3D', 'ShapeNetCore', 'Pix3D'], help='dataset')
...
if opt.dataset == 'Pascal3D':
    ...
    for cat in tqdm(test_cats):
        dataset_test = Pascal3DContrast(root_dir=root_dir, annotation_file=annotation_file,
        ...
        Accs[cat], Meds[cat], cls_errs = test_category(opt.shape, opt.batch_size, opt.dataset, dataset_test, model, opt.bin_size,
                                             cat, predictions_path, logname)
        global_Accs.append(Accs[cat])
        global_Meds.append(Meds[cat])
    ...
    for cat in tqdm(test_cats):
        dataset_test = Pascal3DContrast(root_dir=root_dir, annotation_file=annotation_file, shape_dir=opt.shape_dir,
                                cat_choice=[cat], train=False, random=False, keypoint=False, shape=opt.shape,
                                view_num=opt.view_num, tour=opt.tour, random_model=opt.random_model)
        Accs[cat], Meds[cat], cls_errs = test_category(opt.shape, opt.batch_size, opt.dataset, dataset_test, model, opt.bin_size,
                                             cat, predictions_path, logname)
        global_Accs.append(Accs[cat])
    ...
    test_cats = ['tool', 'misc', 'bookcase', 'wardrobe', 'desk', 'bed', 'table', 'sofa', 'chair']
    for cat in tqdm(test_cats):
...
#            annotation_file = '{}_annotation.txt'.format(cat)
        ...
        dataset_test = Pix3DContrast(train=False, root_dir=root_dir, annotation_file=annotation_file, cls_choice=[cat])
        Accs[cat], Meds[cat], cls_errs = test_category(opt.shape, opt.batch_size, opt.dataset, dataset_test, model, opt.bin_size,
                                             cat, predictions_path, logname)
        global_Accs.append(Accs[cat])
        global_Meds.append(Meds[cat])
...
with open(logname, 'a') as f:
    ...
    f.write('Average for all Samples  >>>>  Med_Err is %.2f, and Acc_pi/6 is %.2f \n' %
