...
import sys
from auxiliary.utils import rotation_err
...
def poseNCE(feat_ori, feat_pos, label, tau=0.1, weighting="linear"):
    ...
    dist = dist.reshape(b, b)  # dist between each other
    ...
    # loss
    # ori image与其positive image进行对应相乘（内积要大），让与其他样本（负样本）之间的内积要小
    loss = - torch.log(logits[:, 0] / torch.sum(logits, -1))
    return loss.mean()

def poseNCE_KD(feat_ori, feat_pos, label, tau=0.1, weighting="linear"):
    # L-2 normalization
    feat_ori = F.normalize(feat_ori, dim=-1)
    feat_pos = F.normalize(feat_pos, dim=-1)
    feat_all = feat_pos.clone()  # only edit here!
    label_all = label.clone()
    b = feat_ori.shape[0]
...
def infoNCE_KD(feat_ori, feat_pos, label, tau=0.1, weighting="linear"):
    '''
    poseNCE between teacher and student
    '''
    # dropout teacher embeddings
    feat_pos = F.dropout(feat_pos, p=0.3, training=True)
...
def singleinfoNCE_KD(feat_ori, feat_pos, label, tau=0.1, weighting="linear"):
    '''
    poseNCE between teacher and student
    '''
...
def multiposeNCE_KD(feat_ori, feat_pos, label, tau=0.1):
    # L-2 normalization
    feat_ori = F.normalize(feat_ori, dim=-1)
    feat_pos = F.normalize(feat_pos, dim=-1)
    feat_all = feat_pos.clone()  # only edited here
    label_all = label.clone()
    b = feat_ori.shape[0]
    ...
    # compare dist with threshold to locate positive samples
    threshold = 30.
    mark_matrix = torch.zeros([b, b], dtype=torch.float).cuda()
    for index_1 in range(b):
#        weight_sum = 1.0  # default weight = 1
        for index_2, d in enumerate(dist[index_1]):
            # looking for positive sample indexs
            if index_2 == index_1:  # exclude itself, default weight = 1
                mark_matrix[index_1][index_2] = 1.0
                continue
            if d <= threshold:
                # calculate relative weight
#                w = 1. - d / threshold
#                mark_matrix[index_1][index_2] = w
#                weight_sum += w
                mark_matrix[index_1][index_2] = 1.0
        # rescale weight
