...
def save_checkpoint_only_resnet(state, is_best, filename, result_path):
    """save state and best model ever"""
    torch.save(state, filename)
    if is_best:  # store model with best accuracy
        shutil.copyfile(filename, os.path.join(result_path, 'model_best_resnet50.pth'))

def save_checkpoint_raw(state, filename):
    torch.save(state, filename)
...
def load_checkpoint(model, pth_file):
    ...
    elif 'net_vp' in checkpoint.keys():
        pretrained_dict = checkpoint['net_vp']
    elif 'student_model' in checkpoint.keys():
        pretrained_dict = checkpoint['student_model']
    else:
...
def get_pred_from_cls_output_contrast(outputs):
    preds = []
    for n in range(0, len(outputs)):
        output = outputs[n]
        _, pred = output.topk(1, 1, True, True)
        preds.append(pred.view(-1)[:pred.size(0)//3])  # only remain studnt outputs before augmentation
    return preds
